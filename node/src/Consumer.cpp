...
Consumer::Consumer(const common::NodeConfiguration &config, Buffer &buffer)
    : loop(ev_loop_new(0)), handler(loop),
      ...
      channel(&connection), buffer(buffer) {
  channel.onError(
...
void Consumer::handleControlMessage(const std::string &data) {
  ...
  const common::itf::Header &header =
      headerSerializer.deserialize(serializedHeader);
  const common::itf::Message &message =
...
void Consumer::createChannel(const std::string &channelToBeCreated,
  ...
  channel.declareQueue(channelToBeCreated, AMQP::durable)
      ...
      .onSuccess([this, author]() {
        if (std::find(clientNames.begin(), clientNames.end(), author) ==
            clientNames.end()) {
...
void Consumer::publish(const common::itf::Block &block,
  ...
  std::scoped_lock lock(mutex);
  spdlog::debug("[{}] Consumer::publish, clientNames size: {}", slot,
               clientNames.size());
  for (const auto &clientName : clientNames) {
    std::string clientChannel = clientName + "_ClientChannel";
    sendDataToClients(clientChannel, block.data, slot);
  }
}

void Consumer::sendDataToClients(
    const std::string &clientChannel,
    const std::vector<common::itf::Message> &messages,
    const std::uint64_t slot) {
  spdlog::debug("[{}] Consumer::publish, messages size: {}", slot,
               messages.size());
  for (const auto &msg : messages) {
    const std::string data = messageSerializer.serialize(msg);
    spdlog::debug("[{}] Consumer::sendDataToClients, data: {}, channel: {}",
                 slot, data, clientChannel);
