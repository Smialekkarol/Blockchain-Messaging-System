...
// void Consumer::run(const common::NodeConfiguration& config, Buffer & buffer)
// {
//     std::thread{ [&config, &buffer]() { Consumer{ config, buffer }.run(); } }.detach();
// }
...
void Consumer::sendDataToClients(const std::string& clientChannel, const std::vector<common::itf::Message>& messages)
{
    serialization::MessageSerializer messageSerializer;
