...
int main(int, char **)
{
    ...
    try
    {
        ...
        // redis.set("key1", "val1");
        // redis.set("key2", "val2");
        auto val1 = redis.get("key1"); // val is of type OptionalString. See 'API Reference' section for details.
        if (val1)
        {
            // Dereference val to get the returned value of std::string type.
            std::cout << "value2: " << *val1 << std::endl;
        } // else key doesn't exist.
        else
        {
            std::cout << "Key1 does not exists\n";
        }

        auto val2 = redis.get("key1"); // val is of type OptionalString. See 'API Reference' section for details.
        if (val2)
        {
            // Dereference val to get the returned value of std::string type.
